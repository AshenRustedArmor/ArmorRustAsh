...
bool function CreateThermiteWallSegment( 
	...
	if ( projectile.proj.savedOrigin != < -999999.0, -999999.0, -999999.0 > ) {
		...
		//regular script path
		if ( !movingGeo ) {
			thermiteParticle = ArmoryFixes_ThermiteTrail( pos, angles, owner, inflictor, duration, FLAME_WALL_FX, damageSource )
			EffectSetControlPointVector( thermiteParticle, 1, projectile.proj.savedOrigin )
			AI_CreateDangerousArea_Static( thermiteParticle, projectile, METEOR_RADIUS_DEF, TEAM_INVALID, true, true, pos )
		} else {
			if ( GetMapName() == "sp_s2s" ) angles = <0,90,0>; // wind dir
			thermiteParticle = ArmoryFixes_ThermiteTrail( pos, angles, owner, inflictor, duration, FLAME_WALL_FX, damageSource, movingGeo  )

			vector relativeDelta = GetRelativeDelta( pos, movingGeo ) // projectile.proj.savedRelativeDelta
			if( movingGeo == projectile.proj.savedMovingGeo ) {
				movingGeo = projectile.proj.savedMovingGeo
				relativeDelta = projectile.proj.savedRelativeDelta
			}
			thread EffectUpdateControlPointVectorOnMovingGeo( thermiteParticle, 1, relativeDelta, movingGeo )
			AI_CreateDangerousArea( thermiteParticle, projectile, METEOR_RADIUS_DEF, TEAM_INVALID, true, true )
		}
		...
		if ( waveCount == 1 ) EmitSoundOnEntity( thermiteParticle, FLAME_WALL_GROUND_BEGINNING_SFX );
		else if ( waveCount == ( maxSegments - 1 ) ) EmitSoundOnEntity( thermiteParticle, FLAME_WALL_GROUND_END_SFX );
		else if ( waveCount == maxSegments / 2  ) EmitSoundOnEntity( thermiteParticle, FLAME_WALL_GROUND_MIDDLE_SFX );
	}
